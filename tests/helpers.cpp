...
TEST(SimdHelpersTest, SimdSumMatchesAccumulate) {
    ...
    const double result = tama::helpers::simdSum(std::span<const double>(values.data(), values.size()));

    EXPECT_NEAR(result, expected, 1e-12);
}

TEST(SimdHelpersTest, SimdSumHandlesTailElements) {
